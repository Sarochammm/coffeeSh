...
$(document).ready( function () {
    ...
    $('#txt_CustomerCode').change (function () {
        ...
        $.ajax({
            url:  '/receipt/customer/detail/' + customer_code,
            type:  'get',
            dataType:  'json',
    ...
    $('#txt_PaymentCode').change (function () {
        ...
        $.ajax({
            url:  '/receipt/payment/detail/' + payment_method,
            type:  'get',
            dataType:  'json',
            success: function  (data) {
                $('#txt_PaymentCode').val(data.payments.payment_code);
                $('#txt_PaymentName').val(data.payments.payment_name);
            },
            error: function (xhr, status, error) {
                $('#txt_PaymentName').val('');
            }
            ...
            success: function  (data) {
                ...
                data.invoices.forEach(invoice => {
                    ...
                    <tr class="d-flex">
                        ...
                        <td class='hide'>${invoice.due_date}</td>
                    </tr>`;
                });
            ...
            success: function  (data) {
                ...
                data.payments.forEach(payment => {
                    ...
                    <tr class="d-flex">
                        ...
                        <td class='col-5'>${payment.payment_name}</td>
                        <td class='col-3'></td>
                        <td class='hide'></td>
                    </tr>`;
                });
                ...
                $('#model_header_2').text('Payment Name');
                $('#model_header_3').text('Note');
    ...
    $('body').on('click', 'a.a_click', function() {
        ...
        var method = $(this).parents('tr').find('td:nth-child(6)').html();
        var ref = $(this).parents('tr').find('td:nth-child(7)').html();
        var received = $(this).parents('tr').find('td:nth-child(8)').html();
        ...
        if ($('#txt_modal_param').val() == 'invoice_no') {
            $("#table_main tbody tr").each(function() {
                if ($(this).find('.order_no').html() == '*') {
                    ...
                    $(this).find('.invoice_amount_remain').html('0.00');
                    $(this).find('.amount_paid_here').html("0");
                    //$(this).find('.payment_reference').html(method);
                }
            });
            ...
            $('#txt_PaymentReference').val(method);
            $('#txt_TotalReceived').val(ref);
            $('#txt_Remarks').val(received);
            ...
            success: function  (data) {
                ...
                data.receipts.forEach(receipt => {
                    ...
                    <tr class="d-flex">
                        ...
                        <td class='col-5'>${receipt.receipt_date}</td>
...
function re_calculate_total_received () {
    ...
    $("#table_main tbody tr").each(function() {
        ...
        if (invoice_no != '') {
            ...
            amount_paid_here = parseFloat(amount_paid_here);
